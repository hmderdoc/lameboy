...
impl PulseChannel {
    ...
    // --- Local patch: expose tone state for the ANSI-music engine. ---
    // See vendor/gameboy_core/PATCH-NOTES.md.

    /// Tone frequency in Hz from the 11-bit timer reload value:
    /// f = 131072 / (2048 - x). Returns 0.0 at the degenerate x == 2048.
    pub fn frequency_hz(&self) -> f64 {
        let x = self.timer_load as f64;
        let denom = 2048.0 - x;
        if denom <= 0.0 {
            0.0
        } else {
    ...
    /// True when the channel is actively producing an audible tone.
    pub fn is_voiced(&self) -> bool {
        self.enabled && self.dac_enabled && self.output_vol > 0
    }
}
