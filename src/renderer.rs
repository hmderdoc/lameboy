...
/// CP437 byte for ▀ (upper half block, 0xDF). Synchronet treats door output as
/// CP437: this byte passes through to CP437 clients and is mapped to U+2580 for
/// UTF-8 clients. Emitting the raw UTF-8 ▀ instead would garble on both.
/// (Local patch -- see PATCH-NOTES.md)
const HALF_BLOCK_UTF8: &[u8] = &[0xDF];
...
impl Renderer {
    ...
    pub fn render(&mut self, fb: &FrameBuffer, stdout: &mut io::Stdout) -> io::Result<()> {
        match self.config.mode {
            RenderMode::Block => self.render_block(fb, stdout),
            RenderMode::Ascii => self.render_ascii(fb, stdout),
        }
    }
    ...
    fn render_block(&mut self, fb: &FrameBuffer, stdout: &mut io::Stdout) -> io::Result<()> {
        self.output_buffer.clear();

        // Use sentinels to force first pixel to set colors
        self.last_fg = 255;
        self.last_bg = 255;

        // Clear entire screen if needed (resize, first frame, etc.)
        if self.needs_clear {
        ...
        let left_col = self.left_pad + 1; // 1-based

        for i in 0..out_rows {
            // Position cursor at the start of each output row
            let row = self.top_pad + i as u16 + 1; // 1-based
            self.move_to(row, left_col);
            ...
            for j in 0..out_cols {
                ...
                self.output_buffer.extend_from_slice(HALF_BLOCK_UTF8);
            }
        }
    ...
    fn render_ascii(&mut self, fb: &FrameBuffer, stdout: &mut io::Stdout) -> io::Result<()> {
        self.output_buffer.clear();

        // Use sentinel to force first pixel to set colors
        self.last_fg = 255;
        ...
        // Black background (256-color index 16 = black)
